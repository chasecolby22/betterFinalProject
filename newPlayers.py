...
class player():
    ...
    def hasMove(self, listOfTiles):
        for i in range(len(self.pieces)):

        
            if len(self.pieces[i].findMoves(listOfTiles[i])) > 0:
                return True
        return False
    ...
    def getCheckers(self, listOfTiles):
        checkers = []
        
        for i in range(len(self.pieces)):
            item = self.pieces[i]
            if item.canMove(listOfTiles[i], self.op.kingTile ):
...
class humanPlayer(player):
    ...
    def finishSelection(self, botString):
        ...
        if self.goodPiece.name() == "king": self.needsKingTile = True
        return [True, False]
